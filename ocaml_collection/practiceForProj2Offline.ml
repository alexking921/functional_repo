...
let nextwhatever =
  match !token with
    |Scanner.SymbolToken name ->  if name = "nil" 
                                  then token := nextToken () ; Nil
                                  else token := nextToken () ; Symbol name
...
let oops message = 
  raise (CantParse message);;

 *)


(*still need to fig out what to put for the makeParser return type *)
module type Parsers =
  sig
    
    val CantParse : string -> string 
    val makeParser : a -> b 
...
(*should be similar to behavior of makeScanner *)
let makeParser = 
  3+7;;
  ...
  (* TYPING OUT ENTIRE makeScanner AND the type 'token' so can see the code more clearly for comparing to makeParser*)
type token =
  | CloseParenToken
  | EndToken
  | NumberToken of int
