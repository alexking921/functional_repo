...
(* QUOTE. Return the single argument without evaluating it. *)
  ...
  primitive "number" 
    (fun args env -> match args with
                    | Cons(a,Nil) -> (match evaluating a env with
                                                | Number a -> tee
                                                | _-> Nil)
                    | _-> raise (EvaluatorError "Error: Strictly 1 argument expected"));;
  ...
  primitive "imply"
    (fun args env -> 
        let rec implying args =
            ...
            | Nil -> tee
            | Cons(arg, Nil) -> evaluating arg env 
            | Cons(arg, args) -> if (evaluating arg env) = Nil 
                                then tee
                                else implying args
            | _-> raise (EvaluatorError "Error: At least 1 argument expected")
        in implying args);;
  ...
  primitive "let"
    (fun args env -> 
        let letting args =
            match args with 
                | Cons(Symbol s, Cons(e1, Cons(e2, Nil))) -> evaluating e2 (envPut s (evaluating e1 env) (env))
                | _ -> raise (EvaluatorError "Error: Expected a Symbol and 2 arguments")
