...
IFIFY: (2 pts off)
-1 point for not recomputing IFIFY BETA in the case of EQUIV. 
-1 point for not sending IF to the correct IF (need recursive calls to ifify).

NORMALIZE: (4 pts off)

-1 point for not testing if π is not another IF. 
	If it's not, then: -1 point for not making a new α by calling NORMALIZE recursively on α. 
-1 point not making a new β by calling NORMALIZE recursively on β. 
-1 point for not returning (if π α β) with the new α, β.




TAUTOLOGY:
tautology: -1 point for not testing if SIMPLIFY returns TRUE using =, not IF-THEN-ELSE.
		MY POINT: I wasn't making the rookie mistake emphasized in lecture about booleans. 
		Because the distinction between True/true False/false, I was making sure it was clear
		to the reader that tautology() is returning the BOOLEAN true and NOT the proposition True.
